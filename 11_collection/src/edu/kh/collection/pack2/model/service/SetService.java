...
public class SetService {
	...
	public void method1() {
		...
		System.out.println("set.size() : " + set.size()); // 10(null 포함)
		
		// 3. boolean remove(E e) :
		// 전달받은 매개변수 e를 Set에서 제거
		// 제거 성공 true, 없으면 false
		System.out.println("set.remove(배달의민족) : " + set.remove("배달의민족")); // true
		System.out.println("set.remove(유플러스) : " + set.remove("유플러스")); // false
		System.out.println(set);
		...
		System.out.println("clear()가 잘 수행됐는지 확인 : " + set.isEmpty());
	}
	
	/**
	 * Set에 저장된 요소(객체)를 꺼내는 방법
	 * 1. Iterator(반복자) 이용
	 * 2. List로 변환 -> 일반 for문 이용
	 * 3. 향상된 for문 이용
	 */
	public void method2() {
