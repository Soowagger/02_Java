...
public class ByteService {
	...
	public void fileByteOutput() {
		...
		try {
			...
			// String 불변성 문제 해결한 객체(가변성)
			StringBuilder sb = new StringBuilder();
			sb.append("Hello World!!\n");
			sb.append("123456789\n");
			sb.append("!@#$%^&*(()_+?><)\n");
			sb.append("가나다라마바사아\n");
			sb.append("선생님 저 공부 그만할게요..");
			sb.append("진짜로");
			sb.append("참말로");
			sb.append("ㅎㅇㅌ");
			...
			*/
			...
			fos.flush(); // 스트림 안에 남아있는 모든 데이터를 모두 밀어낸ㅁ
	...
	public void bufferedFileByteOutput() {
		...
		try {
			...
			bos.write(content.getBytes()); // fos -> bos
			...
			try {
				...
				// 보조스트림(bos) close()시
				// 보조스트림에 생성에 사용된 기반스트림(fos)도 같이 close() 됨
	...
	public void fileByteInput() {
		...
		try {
			...
			while(true) {
				...
				sb.append( (char)value ); // char로 강제형변환해서 글자 형태로 추가
			}
	...
	public void fileCopy() {
		...
		try {
			...
			// 출력용 스트림 생성
			fos = new FileOutputStream(copy); // 경로
			bos = new BufferedOutputStream(fos);
