...
public class PolyService {
	...
	public void ex1() {
		...
		// 에러 발생 : The method setBatteryCapacity(int) is undefined for the type Car
		
		// car3(부모 = 자식(Spark));
		car3.setEngine("경차 엔진");
		car3.setFuel("휘발유");
		car3.setWheel(4);
		// car3.setDiscountOffer(0.5); 동일하게 사용 불가
	...
	// 1) 전달받은 Car 또는 자식객체(Tesla, Spark)의 엔진, 연료, 바퀴개수를 출력하는 메서드
	//    매개변수에 부모타입 참조변수를 작성하면 모든 자식 객체를 전달받을 수 있다.
	public void printCar(Car temp) {
		// 부모타입 참조변수 = 자식타입객체
		// 다형성의 업캐스팅 모양과 똑같다.
		// -> temp에는 Tesla, Spark, Car의 주소가 넘어와도 된다(업 캐스팅)
		// ==> Car temp = new Telsa();
		// ==> Car temp = new Spark();
	...
	public void ex3() {
		...
		// ((자식객체).참조변수) 형식으로 묶어줘야함
		
		Tesla t1 = (Tesla)c1; // (Tesla)c1이 Tesla로 강제형변환 되었기 때문에 Tesla 자료형에 대입해야함
	...
	public void ex5() {
		...
		// 마우스 오버 시 edu.kh.poly.ex1.model.vo.Car.getEngine() 메시지 확인
		
		// 프로그램 "실행 전"
		// - 컴파일러는 getEngine() 메서드가 Car에 있는걸로 인식
		// c1.getEngine() 호출코드와 
		...
		// 업캐스팅 적용 -> 부모 부문만 참조 가능한 상태, (엔진, 연료, 휠)까지만
