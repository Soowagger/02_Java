...
public class ThreadRun {
	/* Thread(스레드) 
	 * - 컴퓨터 프로그램 실행의 기본 단위 중 하나
	 * - 프로세스 내에서 실행되는 흐름의 단위
	 * 
	 * [스레드의 이점]
	 * 1. 스레드를 사용하면 여러 작업을 동시에 실행
	 * -> 시스템의 응답성을 향상시키고, 자원을 효율적으로 활용할 수 있다.
	 * 
	 * 2. 비동기적인 작업 수행이 가능
	 * ex) 네트워크 요청, 파일 입출력과 같은 I/O 작업은 대기 시간 발생할 수 있으므로,
	 * 	   스레드를 사용하여 메인 작업과 병렬로 처리할 수 있음
	 * 
	 * * 병렬 처리 : 하나의 작업을 여러 부분으로 나누어서 동시에 처리하는 것
	 */
