...
public class ArrayExample {
	...
	public void ex1() {
		...
		// 1) 변수 선언
		int num;
		// Stack 영역에 int 자료형을 저장할 수 있는 공간에
		// 4byte를 할당하고 그 공간에 num이라는 이름을 부여했다.
		...
		// ex) int[] arr2 = new double[5]; 에러 발생 
		// 		-> 다른 자료형이기 때문에 연산 불가
	...
	public void ex2() {
		...
		for(int i=0; i < arr.length; i++) { 
			// ** 길이 4 / 배열 끝번호 [3]이기 때문에 <=(이하)가 아닌 <(미만)
			// 0 1 2 3
...
//       int minValue = Arrays.stream(score).min().getAsInt();
	...
	public void ex5() {
		...
		// int형 배열 4칸짜리 생성 후 각 index에 값을 대입하지 않았으므로,
	...
	public void ex8() {
		...
		for(int i=0; i<arr.length; i++) {
			...
			if(arr[i].equals(input)) { // String 비교 시 .equals() 메서드 사용
	...
	public void ex9() {
		...
		} else { // 3. 단, 일치하는 문자 없을 경우 "존재하지 않습니다" 출력
